...
function isCloudFrontBlockedPage({ status, title, bodyText }) {
  const normalizedTitle = normalizeText(title);
  const normalizedBody = normalizeText(bodyText);

  if (normalizedTitle.includes(CLOUDFRONT_BLOCKED_TITLE)) {
    return true;
  }

  const hasCloudFrontMarker =
    normalizedBody.includes("cloudfront") ||
    normalizedBody.includes("generated by cloudfront");
  const hasDeniedMarker =
  ...
  const hasBlockedStatus = Number.isInteger(status) && BLOCKED_STATUS_CODES.has(status);
...
async function getMetadata(urlString) {
  return withPage(async (page) => {
    ...
    const response = await gotoAndWait(page, urlString);
    ...
    const bodyText = await page
      ...
      .catch(() => "");
    if (isCloudFrontBlockedPage({ status: responseStatus, title: pageTitle, bodyText })) {
      throw new Error(
        [
