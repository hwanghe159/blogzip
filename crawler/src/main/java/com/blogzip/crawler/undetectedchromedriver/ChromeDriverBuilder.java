...
public class ChromeDriverBuilder {
  ...
  private ChromeOptions setHostAndPort(ChromeOptions chromeOptions) throws RuntimeException {
    ...
    try {
      ...
      Map<String, Object> experimentals = (Map<String, Object>) experimentalOptions.get(
          chromeOptions);
    ...
    chromeOptions.setExperimentalOption("debuggerAddress",
        debugHost + ":" + String.valueOf(debugPort));
    return chromeOptions;
  }
  ...
  private void handlePrefs(String userDataDir, Map<String, Object> prefs) throws RuntimeException {
    ...
    if (new File(prefsFile).exists()) {
      ...
      try (BufferedWriter bw = new BufferedWriter(
          new FileWriter(prefsFile, StandardCharsets.ISO_8859_1))) {
   ...
   *
   * @param options              chromeOptions           required no prefs args in experiment map,
   *                             if has prefs, use param prefs
  ...
  public ChromeDriver build(ChromeOptions options,
    ...
    try {
      ...
      if (args.contains("--headless") || args.contains("--headless=new") || args.contains(
          "--headless=chrome")) {
        headless = true;
      }
    } catch (Exception ignored) {
  ...
  private String _getChromePath() throws RuntimeException {
    ...
    if (IS_POSIX) {
      ...
      List<String> middles = Arrays.asList(
